...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_hidden=False): # 修改是否返回
        if return_hidden:
            out = x.unsqueeze(0)  # 应该有13层长度
            for resblock in self.resblocks:
                x = resblock(x)
                out = torch.cat((out, x.unsqueeze(0)), dim=0)  # [layers, 192, bs, dim]
            return x, torch.tensor(out)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_hidden=False):
        ...
        if return_hidden:
            x, outs = self.transformer(x, return_hidden)
        ...
        # x = self.ln_post(x[:, 0, :])
        norm_x = self.ln_post(x)

        if self.proj is not None:
            x = norm_x @ self.proj

        # x都是映射后的全局表征，outs是无映射的13层表征
        if return_hidden:
            return x, norm_x, outs
        else:
            return x
...
class CLIP(nn.Module):
    ...
    def build_attention_mask(self):
        ## 从代码上看，文本编码器用的是transformer_decoder，因为attention用的是下三角矩阵，所以只有最后一个token看过所有内容，所以用最后一个
    ...
    def encode_image(self, image, return_hidden=False):
        return self.visual(image.type(self.dtype), return_hidden)

    def encode_text(self, text, return_last_layer=False):
        ...
        if return_last_layer:
            x_transformer_output = x.clone() # clone创建了新的张量同时保持了后续梯度计算 detach().clone()会不受后续梯度影响，
    ...
    def encode_question(self, text):
        ...
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND [77, 64, 512]
        x, out_hiddens = self.transformer(x, return_hidden=True)  # out_hiddens: [layers, 192, bs, dim], x是最后一层输出
        out_hiddens = out_hiddens.permute(0, 2, 1, 3)
        return out_hiddens
            ...
            if hasattr(l, name):
                ...
                if attr is not None:
                    if isinstance(attr, nn.Linear):
                        ...
                        attr.data = attr.data.half()  # nn.parameter
